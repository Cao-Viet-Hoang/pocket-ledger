...
## Money formulas (see `.Codex/rules/money-calculations.md` for full derivations)
...
The full rulebook lives in [.Codex/rules/](.Codex/rules/). The enforceable highlights:
...
7. **Keep docs in sync.** Whenever you add a route, entity, money formula, style rule, data field, or module, update `AGENTS.md` and the relevant file(s) under `.Codex/rules/` in the same change so they always reflect the actual codebase.

See [.Codex/rules/](.Codex/rules/) for the deep-dive rules and [.Codex/agents/](.Codex/agents/) for task-specific subagents.
